...
spritearea=mkrectq('0 0 128 96')
spritearea_w=mkrectq('1 1 128 96')
...
--cell flash--
local celpos=pos2cel(flr((mouse.x+2)/2)+(view.x%4),flr((mouse.y+2)/2)+(view.y%4))
local ccel=currentcel
if ccel.x~=celpos.x or ccel.y~=celpos.y then
ccel.x=celpos.x 
ccel.y=celpos.y
ccel.fls=6
end
...
if ("\n"==keystate or "\n"==keystate) and stat(30)==false then 
 poke(0x5f30,0)
end
...
if mouse.lt then
 if keystate==' ' then
  ...
  if canvas.contp(mouse.x,mouse.y) then
  writing=1
  sfx(48)
  end
  if savebtn.contp(mouse.x,mouse.y) then
  entryname(savepng,'saveto:')
  end
  if loadbtn.contp(mouse.x,mouse.y) then
  entryname(loadpng,'loadfrom:')
  end
  if exptbtn.contp(mouse.x,mouse.y) then
  entryname(exporttxt,'exportto:')
  end
  if cleabtn.contp(mouse.x,mouse.y) then
  entryname(clearsprite,'enter "yes" to clear:','')
  end
  if palette.contp(mouse.x,mouse.y) then
   palettemouse(mouse)
  	if mouse.lt then
    sfx(49)
   end
...
if mouse.l then
 if keystate==' ' then
//cls() stop(mouse.x-mouse.sx)
  local vx=(mouse.x-mouse.sx)/2
  local vy=(mouse.y-mouse.sy)/2
  local pvx=view.stx-(vx>0 and flr(vx) or ceil(vx))
  local pvy=view.sty-(vy>0 and flr(vy) or ceil(vy))

  if pvx~=view.x or pvy~=view.y then
   if viewmove==0 then sfx(50) end
   viewmove=8
  elseif viewmove==0 then
   sfx(-1)
  end
  
  setview(pvx,pvy)

 elseif subview.contp(mouse.x,mouse.y-80+subview.y) and spritearea.contp(mouse.x,mouse.y-80+subview.y)then
  if view.stx~=view.x or view.sty~=view.y then
   view.stx=view.x
   view.sty=view.y
   sfx(57)
  end
  setview(band(mouse.x,bnot(view.w-1)),band(mouse.y-80+subview.y,bnot(view.h-1)))
 elseif writing==1 then
  canvaspset(mouse,premouse)
 end
end

if mouse.lut or mouse.rut then
 sfx(-1)
 if writing==1 then
...
if presskey=="\t" then
 viewsprite=not viewsprite
end

if presskey=='b' and canvas.contp(mouse.x,mouse.y) then
local p=canvaspos(mouse.x,mouse.y)
fillcanvas(colsel,p.x,p.y,view)
sethistory(getrecord())
end

if presskey=='c' then
cprect=mkrect(view.x,view.y,view.w,view.h)
keystate='c'
setmaquee('copied: '..view.x..'-'..view.y..':'..view.w..'-'..view.h,5,6)
return
end
if presskey=='v' then
cpcanvas(cprect,view)
sethistory(getrecord())
setmaquee('pasted: '..view.x..'-'..view.y..':'..view.w..'-'..view.h,6,5)
return
end
if presskey=='z' then
undo()
sfx(51)
elseif presskey=='x' then
redo()
sfx(52)
end
end
...
__map__

